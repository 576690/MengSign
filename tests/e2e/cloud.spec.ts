...
test('cloud school clock is reachable and private APIs reject anonymous requests', async ({ page }) => {
  test.skip(process.env.SCHOOL_SMOKE !== '1', 'Opt-in read-only live school probe');
  await page.goto('/');
  ...
  const checks = await page.evaluate(async () => {
    ...
    const b = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ account: 'x'.repeat(5000), password: 'x' }) });
