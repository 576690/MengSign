...
test("production shell installs and reloads offline without caching API", async ({
  page,
  context,
}) => {
  test.skip(
    process.env.PWA_TEST !== "1",
    "Requires npm run start production server",
  );
  await page.goto("/");
  await expect(
    page.getByRole("button", { name: "体验演示模式" }),
  ).toBeVisible();
