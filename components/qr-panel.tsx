    ...
    async function tick() {
      ...
      try {
        if (!demo && schoolNow(sample, now) === null) {
          ...
          const reading = await api<{ timestamp: number; upstreamRtt: number }>(
            "/api/clock",
          );
          ...
          sample = {
            schoolAtReceive:
              reading.timestamp +
              Math.max(0, roundTrip - reading.upstreamRtt) / 2,
            sampledAt: received,
            rtt: roundTrip,
          };
        ...
        const aligned = demo
          ? Date.now()
          : schoolNow(sample, performance.now());
        ...
        const url = demo
          ? `MengSign 演示二维码 · 非签到码 · ${Math.floor(aligned / 5000)}`
          : qrUrl(key, aligned);
        const data = await QRCode.toDataURL(url, {
          width: 420,
          margin: 2,
          color: { dark: "#133e39", light: "#ffffff" },
        ...
        expires =
          ...
          Math.min(
            ...
            demo
              ? QR_INTERVAL
              : CLOCK_TTL - (performance.now() - sample!.sampledAt),
          );
        setImage(data);
        setError("");
        setRemaining(Math.max(0, expires - performance.now()));
      } catch (e) {
        if (!disposed) {
          failed = true;
    ...
    <div className="qr-layout">
      <section className="panel qr-card">
        <div className="eyebrow">
          ...
          {demo ? "演示二维码 · 无签到效力" : "与学校时间同步"}
        </div>
        <h2>让签到，更轻一点</h2>
        <p className="muted">打开二维码，使用学校客户端扫码</p>
        <div className="qr-frame" aria-live="polite">
          {image && online ? (
            <img
              src={image}
              alt={demo ? "演示二维码，不能用于签到" : "课程签到动态二维码"}
              width={252}
              height={252}
            />
          ) : (
        ...
        <div className="qr-progress">
          <span
            style={{
              width: `${Math.min(100, (remaining / QR_INTERVAL) * 100)}%`,
            }}
          />
        </div>
        <p className="qr-caption">
          {image
            ? `${Math.ceil(remaining / 1000)} 秒后更新${demo ? " · 演示" : ""}`
            : "仅展示有效的二维码"}
        </p>
        {error && (
      ...
      <aside className="stack">
        <section className="panel">
          ...
          <label className="field-label" htmlFor="qr-course">
            生成签到码的课程
          </label>
          <select
            id="qr-course"
          ...
          >
            ...
            <option value="custom">手动输入课程 ID / UUID</option>
          </select>
          {choice === "custom" && (
            <label className="field-label">
              ...
              <input
                ...
                placeholder="7 位课程 ID 或 32 位 UUID"
                maxLength={36}
                autoComplete="off"
              />
            </label>
          )}
          <p className="help">二维码不包含你的账号或会话信息。</p>
        </section>
        <section className="info-card">
          <ShieldCheck size={22} />
          <h3>每一次展示，都有时效</h3>
          <p>
            二维码根据学校时间自动更新。切回页面时会重新校时；网络断开后暂停展示。
          </p>
          {rtt > 0 && !demo && (
            <span className="small muted">本次校时耗时 {rtt} ms</span>
          )}
        </section>
      </aside>
    </div>
