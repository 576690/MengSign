...
export function normalizeDay(day: string) {
  return day.replace(/[-/]/g, "").slice(0, 8);
}
export function courseInstant(day: string, raw: string): number | null {
  ...
  if (!m || !/^\d{8}$/.test(d) || +m[1] > 23 || +m[2] > 59 || +(m[3] || 0) > 59)
    return null;
...
export function todaySorted(courses: Course[], day = shanghaiDay()) {
  return courses
    .filter((c) => normalizeDay(c.day) === day)
    .sort(
      (a, b) =>
        (courseInstant(a.day, a.beginTime) ?? Infinity) -
        (courseInstant(b.day, b.beginTime) ?? Infinity),
    );
}
export function inWindow(c: Course, now: number) {
  const begin = courseInstant(c.day, c.beginTime),
    end = courseInstant(c.day, c.endTime);
  return (
    begin !== null && end !== null && now >= begin - WINDOW_LEAD && now < end
  );
}
export function currentAndNext(courses: Course[], now: number) {
  ...
  const next = today.find(
    (c) =>
      c.id !== current?.id &&
      (!current || c.name !== current.name) &&
      (courseInstant(c.day, c.beginTime) ?? 0) >
        (current
          ? (courseInstant(current.day, current.beginTime) ?? now)
          : now),
  );
  return { current, next };
}
...
export function qrUrl(id: string, timestamp: number) {
  ...
  const param = /^\d{7}$/.test(value)
    ? "courseSchedId"
    : /^[\da-f]{32}$/i.test(compact)
      ? "timeTableId"
      : null;
  ...
  url.searchParams.set(
    param,
    param === "timeTableId" ? compact.toUpperCase() : value,
  );
