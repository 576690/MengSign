...
export function schoolFetch(url: string, init: RequestInit): Promise<Response> {
  return new Promise((resolve, reject) => {
    ...
    req.on('socket', socket => { socket.once('lookup', () => { phase = 'tcp'; }); socket.once('connect', () => { phase = 'tls'; }); socket.once('secureConnect', () => { phase = 'headers'; }); });
    req.on('error', error => { console.warn('school_transport_failure', { phase, code: 'code' in error ? error.code : 'UNKNOWN' }); reject(error); }); req.end(encoded);
  });
}
