...
export function failure(error: unknown) {
  return error instanceof AppError ? json({ error: { code: error.code, message: error.message } }, error.status) : json({ error: { code: 'INTERNAL', message: '服务暂时不可用，请稍后重试' } }, 500);
}
...
export async function body(request: Request) {
  if (!request.headers.get('content-type')?.includes('application/json')) throw new AppError('BAD_REQUEST', '请求格式不正确', 400);
  ...
  while (true) { const { done, value } = await reader.read(); if (done) break; size += value.byteLength; if (size > 4096) { await reader.cancel(); throw new AppError('BAD_REQUEST', '请求内容过大', 413); } chunks.push(value); }
  try { const value = JSON.parse(Buffer.concat(chunks).toString('utf8')); if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(); return value as Record<string, unknown>; }
  catch { throw new AppError('BAD_REQUEST', '请求格式不正确', 400); }
}
